...
namespace Relativity_Extension.CustomPages.NUnit
{
	[TestFixture]
	class ManagerQueueRecordModelTests
	{
		...
		public async Task NewManagerQueueRecordTest_NoAgentIdAsync()
		{
			...
			DataRow dataRow = await DataHelpers.ManagerAgentData.BuildDataRowAsync(dt, row1Id, row1AddedOn, workspaceArtifactId, workspaceName, row1Status, null, row1Priority, row1AddedBy, row1ArtifactId);

			//Act 
			ManagerQueueRecordModel record = new ManagerQueueRecordModel(dataRow, queryMock.Object);
		...
		public async Task NewManagerQueueRecordTest_AgentIdAsync()
		{
			...
			DataRow dataRow = await DataHelpers.ManagerAgentData.BuildDataRowAsync(dt, row1Id, row1AddedOn, workspaceArtifactId, workspaceName, row1Status, row1AgentId, row1Priority, row1AddedBy, row1ArtifactId);
		...
		public async Task NewManagerQueueRecordTest_NoValuesAsync()
		{
			...
			Mock<IQuery> queryMock = new Mock<IQuery>();
			DataTable dt = await DataHelpers.ManagerAgentData.BuildDataTableAsync();
			DataRow dataRow = await DataHelpers.ManagerAgentData.BuildEmptyDataRowAsync(dt);
